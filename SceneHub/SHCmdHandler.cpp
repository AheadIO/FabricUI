...
std::string SHCmdHandler::encodeRTValToJSON(FabricCore::Context const& context, FabricCore::RTVal const& rtVal) {
  if(rtVal.isValid())
  {
    if(rtVal.isObject())
    {
...
void SHCmdHandler::decodeRTValFromJSON(FabricCore::Context const& context, FabricCore::RTVal & rtVal, FTL::CStrRef json) {
  if(json.size() > 2)
  {
    try
    {
